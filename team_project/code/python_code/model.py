...
# ---------------------------------------------------------
df = pd.read_csv("Data/tulika_dataframe.csv")
#%%
...
target_col = 'brca1'
id_cols = ['depmap_id', 'cell_line_display_name', 'lineage_1']
...
plt.title("Top 20 Genetic Predictors of BRCA1 Dependency\nPositive = Co-dependency | Negative = Compensatory/Resistance")
plt.grid(axis='y')
# Save plot
plt.tight_layout()
...
plt.scatter(test_predictions['brca1'], test_predictions['.pred'], 
            alpha=0.6, s=50, color="#2c3e50")
# Add diagonal line
limits = [min(min(test_predictions['brca1']), min(test_predictions['.pred'])),
          max(max(test_predictions['brca1']), max(test_predictions['.pred']))]
plt.plot(limits, limits, ls="--", c="firebrick", lw=2)

# Annotation for R2
plt.text(x=max(test_predictions['brca1']), y=min(test_predictions['.pred']), 
         s=f"R² = {r2_val:.2f}", 
         ha='right', va='bottom', fontsize=14, fontweight='bold')

plt.xlabel("Actual BRCA1 Dependency Score (CRISPR)")
...
binary_truth = (test_predictions['brca1'] < -0.5).astype(int) 
...
residuals = test_predictions['brca1'] - test_predictions['.pred']
