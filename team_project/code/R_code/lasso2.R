...
df<-read.csv("processed_df_with_all_target_genes.csv")
#df<-read.csv("tulika_dataframe.csv")
df <- df[df$lineage_1_x == "Breast", ]
set.seed(222)

df_split <- initial_split(df, prop = 3/4, strata = PARP1)
...
lasso_rec <-
  recipe(PARP1 ~ ., data = df_train) %>%
  update_role(depmap_id, cell_line_display_name_x, lineage_1_x, BRCA2, BRCA1, TP53, BRIP1, new_role = "ID") %>%
...
ggplot(test_predictions, aes(x = PARP1, y = .pred)) +
...
cor_val <- cor(test_predictions$PARP1, test_predictions$.pred)
r2_val <- cor_val^2
...
ggplot(test_predictions, aes(x = PARP1, y = .pred)) +
  ...
  annotate("text", x = max(test_predictions$PARP1), y = min(test_predictions$.pred), 
...
roc_data <- test_predictions %>%
  mutate(
    ...
    truth_class = factor(ifelse(PARP1 < -0.5, "Dependent", "Not_Dependent"), 
...
ggplot(test_predictions, aes(x = .pred, y = PARP1 - .pred)) +
